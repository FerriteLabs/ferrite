...
> Replace `VERSION` below with the actual release version (e.g., `0.3.0`).

## Pre-Release

- [ ] All CI checks pass: `cargo fmt --all --check && cargo clippy --workspace -- -D warnings`
- [ ] All tests pass: `cargo test --workspace --lib` (expect 6,000+ passing)
...
- [ ] CHANGELOG.md `[Unreleased]` section moved to `[VERSION]` with release date
- [ ] Version bumped in workspace `Cargo.toml`: `version = "VERSION"`
- [ ] README quick-start instructions verified on fresh machine
- [ ] License headers present in all source files
- [ ] `ferrite-ops/Dockerfile` ARG `FERRITE_VERSION` matches VERSION
...
- [ ] Verify `cargo install ferrite` works
