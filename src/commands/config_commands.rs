...
pub fn config_set_extended(param: &str, value: &str) -> Frame {
    match param {
        ...
        "activedefrag" | "lazyfree-lazy-expire" => match value.to_lowercase().as_str() {
            "yes" | "no" => Frame::simple("OK"),
            _ => Frame::error(format!(
                "ERR Invalid argument '{value}' for CONFIG SET '{param}'"
            )),
        },
        _ => Frame::error(format!("ERR Unsupported CONFIG parameter: {param}")),
    }
}
...
pub fn command_getkeys(args: &[String]) -> Frame {
    ...
    let keys: Vec<Frame> = match cmd.as_str() {
        ...
        | "STRLEN" | "GETRANGE" | "SETRANGE" | "SETNX" | "GETSET" | "LPUSH" | "RPUSH" | "LPOP"
        | "RPOP" | "LLEN" | "LRANGE" | "LINDEX" | "LSET" | "SADD" | "SREM" | "SCARD"
        | "SMEMBERS" | "SISMEMBER" | "SRANDMEMBER" | "SPOP" | "ZADD" | "ZREM" | "ZSCORE"
        | "ZCARD" | "ZRANGE" | "ZRANK" | "HSET" | "HGET" | "HDEL" | "HLEN" | "HGETALL"
        | "HKEYS" | "HVALS" | "XADD" | "XLEN" | "XRANGE" | "XREVRANGE" | "XREAD" | "XINFO"
        | "XDEL" | "XTRIM" => {
            if args.len() > 1 {
