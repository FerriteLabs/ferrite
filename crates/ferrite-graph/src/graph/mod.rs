...
impl Graph {
    ...
    pub fn cypher_query(&self, cypher: &str) -> Result<QueryResult> {
        ...
        let stmt =
            CypherParser::parse(cypher).map_err(|e| GraphError::InvalidQuery(e.to_string()))?;
        match stmt {
            cypher::CypherStatement::Query(ref q) => {
                ...
                cypher::execute_read_only(q, &self.storage.read())
                    .map_err(|e| GraphError::InvalidQuery(e.to_string()))
            }
    ...
    pub fn cypher_execute(&self, cypher: &str) -> Result<QueryResult> {
        ...
        self.metrics
            .queries
            .fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        cypher::execute(&stmt, &mut self.storage.write())
            .map_err(|e| GraphError::InvalidQuery(e.to_string()))
    }
